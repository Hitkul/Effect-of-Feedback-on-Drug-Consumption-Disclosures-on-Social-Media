...
for train_idx, val_idx in kfold.split(trainX_text, trainY):
    ...
    checkpoint = ModelCheckpoint(f'../model_checkpoints/drug_consumption_classification/BERT/{fold_no}.hdf5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max', save_weights_only = True)
    ...
    model.load_weights(f'../model_checkpoints/drug_consumption_classification/BERT/{fold_no}.hdf5')
...
checkpoint = ModelCheckpoint(f'../model_checkpoints/drug_consumption_classification/BERT/final_model.hdf5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max', save_weights_only = True)
early_stopping = EarlyStopping(monitor='val_accuracy', verbose=1, patience=5, mode='max', restore_best_weights=True)
...
model.load_weights(f'../model_checkpoints/drug_consumption_classification/BERT/final_model.hdf5')
...
with open(f'../results/drug_consumption_classification/BERT.json','w') as fp:
