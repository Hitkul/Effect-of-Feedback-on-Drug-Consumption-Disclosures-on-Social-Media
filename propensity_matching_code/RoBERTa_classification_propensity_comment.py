...
try:
    results_dict = json.load(open(f'../results/propensity_models/RoBERTa_classification/comments/{subreddit}.json','r'))
except:
    results_dict = {}
...
Y = sample_data['no_of_comments'].to_list()
...
checkpoint = ModelCheckpoint(f'../model_checkpoints/RoBERTa_propensity_classification/{subreddit}_comment_th_{high_feedback_th}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='min', save_weights_only = True)
...
with open(f'../results/propensity_models/RoBERTa_classification/comments/{subreddit}.json','w') as fp:
