...
n = int(sys.argv[3])
propensity_method = "RoBERTa_propensity_classification_score"
print(f"Subreddit : {subreddit} | High Th : {high_th} | N : {n} | propensity method : {propensity_method}")


if not os.path.exists(f"../results/matching_quality/{subreddit}/scores"):
    os.makedirs(f"../results/matching_quality/{subreddit}/scores")

try:
    results_dict = json.load(open(f'../results/matching_quality/{subreddit}/scores/{propensity_method}.json','r'))
except:
    results_dict = {}
...
if f'{propensity_method}_th_{high_th}' not in dc_p_data.columns:
    print("Merging to add score propensity ")
    raw_data = pd.read_pickle(f"../processed_data/propensity_dataframes/{subreddit}.p")
    dc_p_raw = raw_data[(raw_data['drug consumption']==1) & (raw_data['type']=='P')]
    dc_p_data = pd.merge(dc_p_data, dc_p_raw[["id", f'{propensity_method}_th_{high_th}']], on="id", how="left")
    print("Dumping new to file")
    dc_p_data.to_pickle(f"../processed_data/matching_dataframe/{subreddit}.p")
...
def get_treatment(row):
    ...
    score_values = before_data['score'].values
    return all([i>=high_th for i in score_values])
...
try:
    ...
    with open(f'../results/matching_quality/{subreddit}/scores/{propensity_method}.json','w') as fp:
...
nn_features = json.load(open(f"../processed_data/propensity_model_features/score/{subreddit}_th_{high_th}.json",'r'))
